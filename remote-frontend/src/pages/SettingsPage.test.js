...
describe("settings page", () => {
  ...
  test.only("should retrieve initial config values from api", async () => {
    ...
    expect((await screen.findByLabelText(/Mouse sensitivity/i)).value).toBe(
      "1"
    );
  ...
  test("should save the user applied settings", () => {
    ...
    fireEvent.change(screen.getByLabelText(/Scroll sensitivity/i), {
      target: { value: "0.7" },
    });
    fireEvent.change(screen.getByLabelText(/Mouse sensitivity/i), {
      target: { value: "0.3" },
    });
  ...
  describe("Scroll sensitivity slider", () => {
    test("should have a slider for scroll sensitivity with a max value of 5", () => {
      ...
      expect(screen.getByLabelText(/Scroll sensitivity/i).max).toBe(
        SLIDER_MAX.toString()
      );
    });
  ...
  describe("Mouse sensitivity slider", () => {
    test("should have a slider for mouse sensitivity with a max value of 5", () => {
      ...
      expect(screen.getByLabelText(/Mouse sensitivity/i).max).toBe(
        SLIDER_MAX.toString()
      );
    });
